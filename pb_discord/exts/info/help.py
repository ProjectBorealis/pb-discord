...
from pb_discord.pagination import LinePaginator
from pb_discord.utils.messages import wait_for_deletion
...
class SubcommandButton(ui.Button):
    ...
    async def callback(self, interaction: Interaction) -> None:
        ...
        if isinstance(subcommand, Group):
            embed, subcommand_view = await self.help_command.format_group_help(
                subcommand
            )
        else:
            embed, subcommand_view = await self.help_command.command_formatting(
                subcommand
            )
...
class GroupButton(ui.Button):
    ...
    async def callback(self, interaction: Interaction) -> None:
        ...
        embed, group_view = await self.help_command.format_group_help(
            self.command.parent
        )
...
class CommandView(ui.View):
    ...
    def __init__(
        self, help_command: CustomHelpCommand, command: Command, context: Context
    ):
...
class GroupView(CommandView):
    ...
    def __init__(
        ...
        if (
            len(subcommands) + len(self.children)
            > self.MAX_ROWS * self.MAX_BUTTONS_IN_ROW
        ):
            log.trace(
                f"Attempted to add navigation buttons for `{group.qualified_name}`, but there was no space."
            )
            return
...
class CustomHelpCommand(HelpCommand):
    ...
    async def command_not_found(self, query: str) -> HelpQueryNotFoundError:
        ...
        return HelpQueryNotFoundError(
            f'Query "{query}" not found.', {choice[0]: choice[1] for choice in result}
        )
    ...
    async def command_formatting(
        self, command: Command
    ) -> tuple[Embed, CommandView | None]:
        """
        ...
        if arguments:
            ...
            for argument in arguments:
                ...
                name = f"{name:<{width}}".replace(" ", " ")
                entry = (
                    f'{2 * " "}{name} {argument.description or "No description given."}'
                )
                n = 48
                if len(entry) > n:
                    lines = wrap(entry, n)
        ...
        view = (
            CommandView(self, command, self.context)
            if not self.context.command_failed
            else None
        )
        return embed, view
    ...
    async def format_group_help(self, group: Group) -> tuple[Embed, CommandView | None]:
        ...
        # If the help is invoked in the context of an error, don't show subcommand navigation.
        view = (
            GroupView(self, group, commands_, self.context)
            if not self.context.command_failed
            else None
        )
        return embed, view
