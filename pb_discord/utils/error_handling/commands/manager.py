...
from pb_discord.utils.error_handling.commands import AbstractCommandErrorHandler
from pb_discord.utils.logging import get_logger
...
class CommandErrorManager:
    ...
    async def handle_error(
        self, error: Exception, context_or_interaction: Context | Interaction
    ) -> None:
        """
    ...
    def register_handler(self, handler: AbstractCommandErrorHandler) -> None:
        ...
        if handler_name in self._registered_handlers:
            log.info(
                f"Skipping registration of command error handler {handler_name!r} as it's already registered."
            )
            return
    ...
    def _get_callback(
        ...
        Callable[[Context, Exception], NoReturn]
        | Callable[[Interaction, Exception], NoReturn]
        | None
    ):
        ...
        raise ValueError(
            f"Expected Context or Interaction, got {type(context_or_interaction).__name__}"
        )
