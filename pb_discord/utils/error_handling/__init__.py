...
async def handle_forbidden_from_block(
    error: Forbidden, message: Message | None = None
) -> None:
    """
    ...
    if not message:
        log.info(
            "Failed to add reaction(s) to a message since the message author has blocked the bot"
        )
        return
