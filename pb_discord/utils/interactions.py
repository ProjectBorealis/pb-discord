...
from pb_discord.utils.scheduling import create_task
...
def user_has_access(
    ...
    *,
    ...
    return bool(
        ...
        or any(role.id in allowed_roles for role in getattr(user, "roles", []))
    )


async def _handle_modify_message(
    message: Message, action: Literal["edit", "delete"]
) -> None:
    ...
    try:
        ...
        if e.code == 50083:
            log.debug(
                f"Could not {action} message {message.id} due to it being in an archived thread."
            )
        elif isinstance(e, NotFound):
            log.info(
                f"Could not find message {message.id} when attempting to {action} it."
            )
        else:
            log.exception(
...
class ViewWithUserAndRoleCheck(ui.View):
    ...
    async def interaction_check(self, interaction: Interaction) -> bool:
        ...
        await interaction.response.send_message(
            "This is not your button to click!", ephemeral=True
        )
        return False
