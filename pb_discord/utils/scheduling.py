...
from pb_discord.utils import logging
...
class Scheduler:
    ...
    def schedule(self, task_id: abc.Hashable, coroutine: abc.Coroutine) -> None:
        ...
        task = asyncio.create_task(
            _coro_wrapper(coroutine), name=f"{self.name}_{task_id}"
        )
    ...
    def schedule_at(
        self, time: datetime, task_id: abc.Hashable, coroutine: abc.Coroutine
    ) -> None:
        """
        ...
        """
        now_datetime = (
            datetime.now(time.tzinfo) if time.tzinfo else datetime.now(tz=UTC)
        )
    ...
    def schedule_later(
        self, delay: float, task_id: abc.Hashable, coroutine: abc.Coroutine
    ) -> None:
        """
    ...
    def _task_done_callback(
        self, task_id: abc.Hashable, done_task: asyncio.Task
    ) -> None:
        """
