...
from pb_discord.constants import DEBUG_MODE, RedirectOutput
from pb_discord.log import get_logger
from pb_discord.utils import function, scheduling
from pb_discord.utils.checks import ContextCheckFailure, in_whitelist_check
from pb_discord.utils.function import command_wraps
...
def in_whitelist(
    *,
    ...
    redirected to the `redirect` channel that was passed or simply told that they're not
    allowed to use this particular command (if `None` was passed, default).
    """

    def predicate(ctx: Context) -> bool:
        """Check if command was issued in a whitelisted context."""
        return in_whitelist_check(
            ctx, channels, categories, roles, redirect, fail_silently
        )
...
def not_in_blacklist(
    *,
    ...
    def predicate(ctx: Context) -> bool:
        ...
        not_blacklisted = not in_whitelist_check(
            ctx, channels, categories, roles, fail_silently=True
        )
...
def respect_role_hierarchy(member_arg: function.Argument) -> t.Callable:
    ...
    def decorator(func: types.FunctionType) -> types.FunctionType:
        ...
        async def wrapper(*args, **kwargs) -> t.Any:
            ...
            log.trace(
                f"{func.__name__}: {target.top_role=} < {actor.top_role=}; calling func"
            )
            return await func(*args, **kwargs)
